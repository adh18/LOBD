...
if isfile('fokker_planck_data.mat')
    load('fokker_planck_data_final.mat')
else
    run('fokker_planck_simululations')
end
...
sols = {sol1(:, tstart:815), sol4(:, tstart:815)};
usedsols = {'sol1', 'sol2'};
[lobd, output] = LOBD(sols, R, 'useminf', false, 'maxiters', 50, 'cgiters', 100, 'showevery', 1, 'nonneg', true);
[lobd, output] = LOBD(sols, R, 'useminf', false, 'maxiters', 15, 'cgiters', 1000, 'showevery', 1, 'nonneg', true, 'initialvalues', lobd);
%% Calculate the exact DMD of the same input data
[dmdX, dmdT, ~, ~, bs] = exactDMD(sols, R, t(2) - t(1));

%% Predictions
newic = sol2(:, tstart);
newsol = sol2(:, tstart:815);
...
figure 
subplot(2, 5, 1)
imagesc(t, x, sols{1}); caxis([0, 0.015]); ylabel('Position x')
subplot(2, 5, 2)
imagesc(t, x, sols{2}); caxis([0, 0.015]);
subplot(2, 5, 6);
imagesc(t, x, newsol); caxis([0, 0.015]); xlabel('Time t'); ylabel('Position x')
subplot(2, 5, 7);
imagesc(t, x, lobdpred); caxis([0, 0.015]); xlabel('Time t');
subplot(2, 5, 8);
imagesc(t, x, real(dmdpred)); caxis([0, 0.015]); xlabel('Time t');
subplot(2, 5, 9);
...
save('fokker_planck_lobdresults_final.mat', 'tstart', 'R', 'lobd', 'usedsols', ...
